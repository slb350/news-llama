...
class NewsLlama:
    ...
    def __init__(self, config_path: str = None, user_interests: List[str] = None):
        # Skip default sources if user interests are specified
        skip_defaults = bool(user_interests)
        ...
        # Initialize source discovery if user interests are provided
        if self.user_interests and self.config.source_discovery.enabled:
    ...
    async def initialize(self) -> None:
        ...
        if self.source_discovery and self.user_interests:
            logger.info(f"Discovering sources for interests: {self.user_interests}")
            discovered_sources = await self.source_discovery.discover_sources(self.user_interests)
            self.config.discovered_sources = discovered_sources
            
            # Add dynamic aggregator for discovered sources
            self.aggregators['dynamic'] = DynamicAggregator(self.config, discovered_sources)
            
            logger.info(f"Added dynamic aggregator with {len(discovered_sources)} discovered sources")
    ...
    def _filter_top_articles(self, articles: List, max_articles: int = 20) -> List:
        ...
        for article in articles:
            ...
            if article.published_at:
                ...
                recency_score = max(0, 10 - (age_hours / 24))  # Linear decay over 10 days
                score += recency_score
            ...
            if any(trusted in source_lower for trusted in ['reddit', 'techcrunch', 'ars technica', 'github']):
                score += 5
    ...
    def _prefilter_articles_for_summarization(self, articles: List) -> List:
        ...
        logger.info(f"Pre-filtering to top {max_per_category} articles per category before summarization")
        ...
        for category, cat_articles in by_category.items():
            ...
            for article in cat_articles:
                ...
                if article.published_at:
                    ...
                    if pub_at.tzinfo is None or pub_at.tzinfo.utcoffset(pub_at) is None:
                        pub_at = pub_at.replace(tzinfo=timezone.utc)
                    age_hours = (now - pub_at).total_seconds() / 3600
                    recency_score = max(0, 10 - (age_hours / 2.4))  # Linear decay over 24 hours
                    score += recency_score
            ...
            logger.info(f"Category '{category}': filtered from {len(cat_articles)} to {len(top_articles)} articles")

        logger.info(f"Pre-filtered from {len(articles)} to {len(filtered_articles)} articles total")
    ...
    def _filter_valid_summaries(self, articles: List) -> List:
        ...
        for article in articles:
            ...
            if hasattr(article, 'importance_score') and article.importance_score < 0.6:
                logger.debug(f"Filtering out article with low importance score: {article.title[:50]}...")
                continue

            # Check for empty key points
            if hasattr(article, 'key_points') and not article.key_points:
                logger.debug(f"Filtering out article with empty key points: {article.title[:50]}...")
                continue
            ...
            if hasattr(article, 'ai_summary') and article.ai_summary:
                ...
                if any(indicator in article.ai_summary.lower() for indicator in failure_indicators):
                    logger.debug(f"Filtering out article with failed extraction: {article.title[:50]}...")
                    continue
    ...
    async def run(self) -> None:
        ...
        try:
            ...
            articles_to_summarize = self._prefilter_articles_for_summarization(unique_articles)

            # Generate summaries only for pre-filtered articles
            logger.info(f"Summarizing {len(articles_to_summarize)} pre-filtered articles")
            summarized_articles = await self.summarizer.summarize_batch(articles_to_summarize)

            # Filter out articles with failed content extraction
            valid_articles = self._filter_valid_summaries(summarized_articles)
            logger.info(f"Filtered to {len(valid_articles)} valid articles (removed {len(summarized_articles) - len(valid_articles)} failed extractions)")
            ...
            return {
                ...
                'discovered_sources': len(self.config.discovered_sources) if hasattr(self.config, 'discovered_sources') else 0
            }
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="News Llama - AI-powered news curation")
    parser.add_argument(
        "--schedule",
        action="store_true",
