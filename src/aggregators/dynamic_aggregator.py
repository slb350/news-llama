...
class DynamicAggregator(BaseAggregator):
    ...
    async def collect(self) -> List[Article]:
        ...
        try:
            ...
            for source_type, sources in sources_by_type.items():
                try:
                    type_articles = await self._collect_from_source_type(source_type, sources)
                    articles.extend(type_articles)
                    logger.info(f"Collected {len(type_articles)} articles from {source_type} discovered sources")
                except Exception as e:
                    logger.error(f"Error collecting from {source_type} discovered sources: {e}")
    ...
    async def _collect_from_source_type(self, source_type: str, sources: List[DiscoveredSource]) -> List[Article]:
    ...
    async def _collect_from_rss_sources(self, sources: List[DiscoveredSource]) -> List[Article]:
        """Collect from RSS sources"""
        articles = []
        rss_config = self.config.sources.get('rss', [])
    ...
    async def _collect_from_twitter_sources(self, sources: List[DiscoveredSource]) -> List[Article]:
        ...
        for source in sources:
            ...
            try:
                ...
                logger.warning(f"Error collecting from Twitter source {source.name}: {e}")
        
        return articles
    
    async def _collect_from_reddit_sources(self, sources: List[DiscoveredSource]) -> List[Article]:
        ...
        for source in sources:
            ...
            try:
                ...
                logger.warning(f"Error collecting from Reddit source {source.name}: {e}")

        return articles

    async def _collect_from_reddit_search(self, sources: List[DiscoveredSource]) -> List[Article]:
        ...
        for source in sources:
            try:
                ...
                search_results = await reddit_aggregator.search_reddit(query, source.category, limit=25)
                articles.extend(search_results)
                logger.info(f"Got {len(search_results)} results from Reddit search for '{query}'")
            except Exception as e:
                logger.warning(f"Error searching Reddit for '{source.category}': {e}")

        return articles

    async def _collect_from_web_search(self, sources: List[DiscoveredSource]) -> List[Article]:
